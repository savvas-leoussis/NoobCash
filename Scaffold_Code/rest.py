...
def initiate_node_id():
    ...
    if (node.total_nodes == next_id):
        ...
        node.extend_ring(next_id, port+next_id, next_public_key, [transaction_output_next])
...
def broadcast_ring_and_blockchain():
    ...
    for peer_id, peer in node.ring.items():
        if peer_id != 0:
            r = requests.post('http://127.0.0.1:{}/initialize'.format(peer['port']), json=info_json)

            if (r.status_code != 200):
                print("EXIT EVERYTHING. It has developed artificial intelligence!")
                # for peer_id, peer in node.ring.items():
                #     if peer_id == 0:
                #         requests.post('http://127.0.0.1:{}/exit'.format(peer['port']), json=ring_json)
...
def init_ring_and_chain():
    ...
    if node.node_lock.acquire():
        for peer_id, peer in ring_dict.items():
            ...
            ring_formatted[int(peer_id)] = {'port': peer['port'],
                                            'public_key': peer['public_key'],
                                            'utxos': utxos_list,
...
def receive_transaction():
    ...
    if node.node_lock.acquire():
        ...
        if result:
            ...
            if len(node.blockchain.unconfirmed_transactions) == node.blockchain.chain[0].capacity:
...
if __name__ == '__main__':
    ...
    # url = 'http://127.0.0.1:{}/home'.format(port+node.id)
    # threading.Timer(1.25, lambda: webbrowser.open_new_tab(url)).start()
    if (node.id == node.total_nodes - 1):
        print("WAT")
        threading.Timer(1.25, lambda: requests.post("http://127.0.0.1:5000/start_initialization")).start()
        # requests.post("http://127.0.0.1:5000/start_initialization")
    app.run(host='127.0.0.1', port=port+node.id)

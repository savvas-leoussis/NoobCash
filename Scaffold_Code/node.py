...
class Node:
	def __init__(self):
		...
		try:
			r = requests.post("http://127.0.0.1:5000/login", data = {'public_key': self.wallet.get_public_key()})
			...
			self.extend_ring(0, 5000, self.wallet.get_public_key(), [new_transaction_output])
	...
	def extend_ring(self, id, port, public_key, utxos):
		# self.ring.extend([{'id': id,
		# 				   'port': port,
		# 				   'public_key': public_key,
		# 				   'balance': balance}])
		# print(self.ring)
		self.ring[id] = {'port': port,
						 'public_key': public_key,
						 'utxos': utxos,
	...
	def create_transaction(self, recipient_id, amount):
		...
		if self.node_lock.acquire():
			...
			if len(self.blockchain.unconfirmed_transactions) == self.blockchain.chain[0].capacity:
	...
	def broadcast_transaction(self, recipient_id, transaction):
		...
		for peer_id, peer in self.ring.items():
			if peer_id != self.id:
				threading.Thread(target = (lambda: requests.post("http://127.0.0.1:{}/add_transaction".format(peer['port']), json=info_json))).start()
	...
	def broadcast_block(self, block):
		...
		for peer_id, peer in self.ring.items():
			if peer_id != self.id:
				threading.Thread(target = (lambda: requests.post("http://127.0.0.1:{}/add_block".format(peer['port']), json=info_json))).start()
	...
	def resolve_conflicts(self):
		...
		for peer_id, peer in self.ring.items():
			r = requests.get('http://127.0.0.1:{}/get_chain_len'.format(peer['port']))
			if (r.status_code == 200):
		...
		if my_len < max_len:
			r = requests.get('http://127.0.0.1:{}/get_chain'.format(self.ring[max_id]['port']))
			if (r.status_code == 200):
